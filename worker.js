...
async function ensurePurchaseTable(db) {
  await db.prepare(`
    CREATE TABLE IF NOT EXISTS energy_purchases (
      ...
      PRIMARY KEY (Username, PurchaseDate),
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/leaderboard' && request.method === 'GET') {
      try {
        const { results } = await env.DB.prepare(`
          SELECT Username, D2E, SteemReward, Energy
          FROM accounts
    ...
    if (url.pathname === '/api/reward' && request.method === 'POST') {
      let body;
      try { body = await request.json(); } catch { return json({ error: 'Invalid JSON' }, 400, responseOrigin); }
    ...
    // POST /api/shop/energy
    // Atomically charges 10 D2E and adds 1 Energy. Maximum 10 purchases per UTC day.
    if (url.pathname === '/api/shop/energy' && request.method === 'POST') {
      ...
      try {
        ...
        const purchasedToday = Math.max(0, Number(purchases?.Quantity) || 0);
        const d2e = Math.max(0, Number(account.D2E) || 0);
        ...
        const results = await env.DB.batch([
          env.DB.prepare(`
            UPDATE accounts SET D2E = D2E - ?, Energy = Energy + 1
            WHERE Username = ? AND D2E >= ?
          `).bind(ENERGY_PRICE, username, ENERGY_PRICE),
          env.DB.prepare(`
            INSERT INTO energy_purchases (Username, PurchaseDate, Quantity)
            VALUES (?, ?, 1)
            ON CONFLICT(Username, PurchaseDate) DO UPDATE SET Quantity = Quantity + 1
          `).bind(username, date)
        ]);

        if (!results?.[0]?.meta?.changes) {
          return json({ success: false, error: 'Purchase could not be completed' }, 409, responseOrigin);
        }
        ...
        console.error(error);
        return json({ error: 'Unable to complete energy purchase' }, 503, responseOrigin);
      }
    }
