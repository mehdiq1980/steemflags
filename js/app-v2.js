...
async function start(){
 ...
 function renderQuestion(q,p,n){counter.textContent=`${t('question')} ${n} ${t('of')} ${game.totalQuestions}`;score.textContent=`${t('score')}: ${p}`;progressBar.style.width=`${n/game.totalQuestions*100}%`;flag.textContent=q.country[1];answers.replaceChildren();feedback.textContent='';feedback.className='feedback';next.hidden=true;next.disabled=false;hideMoreInformation();q.options.forEach(([name])=>{const b=document.createElement('button');b.type='button';b.className='answer';b.dataset.canonical=name;b.textContent=name;b.onclick=()=>choose(b);answers.appendChild(b)})}
 ...
 function choose(btn){const canonical=btn?.dataset?.canonical;if(!canonical)return;const r=game.answer(canonical);if(!r)return;if(r.correct){sessionCorrect++;gameScore++;feedback.textContent='✅ Correct Answer: +1 D2E';btn.classList.add('correct')}else{gameScore--;feedback.textContent=`❌ Wrong Answer: -1 D2E\n${r.answer}`;btn.classList.add('wrong')}for(const b of answers.children){b.disabled=true;if(b.dataset.canonical===r.answer)b.classList.add('correct')}score.textContent=`${t('score')}: ${gameScore}`;showMoreInformation();if(r.complete){next.hidden=true;next.disabled=true;clearSavedGame(username);void finishGame()}else{next.hidden=false}}
 async function finishGame(){
  if(!username||!game.isComplete())return;
  if(next){next.hidden=true;next.disabled=true}
  if(feedback)feedback.setAttribute('aria-busy','true');
  const perfect=sessionCorrect===game.totalQuestions;
  const eventId=`${username}-${Date.now()}-${crypto.randomUUID()}`;
  try{
   const result=await saveGameResult({username,score:gameScore,perfect,eventId});
   if(!result.success)throw Error(result.error||'Unable to save game result.');
   account=result.account;
   applyAccount();
   clearSavedGame(username);
   saveRewardState({username,gameReward:Number(result.score)||gameScore,perfectBonus:Number(result.perfectBonus)||(perfect?4:0),adReward:0,context:'game',pendingSave:false,score:gameScore,perfect,eventId});
   showHome();
  }catch(e){
   clearSavedGame(username);
   saveRewardState({username,gameReward:gameScore,perfectBonus:perfect?4:0,adReward:0,context:'game',pendingSave:true,score:gameScore,perfect,eventId});
   if(feedback){feedback.textContent='';feedback.className='feedback'}
   showHome();
  }finally{if(feedback)feedback.removeAttribute('aria-busy')}
 }
 function showHome(){loginView.hidden=true;home.hidden=false;leaderboard.hidden=false;gameView.hidden=true;updateResume();window.dispatchEvent(new CustomEvent('steemflags:show-home'))}
 ...
 if(language){language.onchange=()=>{const lang=setLanguage(language.value);applyLanguage(document,lang);if(!gameView.hidden&&game.current)renderQuestion(game.current,game.score,game.questionNumber)}}
