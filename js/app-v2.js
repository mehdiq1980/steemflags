...
async function loadComponent(){const r=await fetch(`./components/app-shell.html?v=20260819-10`,{cache:'no-store'});if(!r.ok)throw Error(`Unable to load component: ${r.status}`);return r.text()}
...
function choose(button,name){if(game.answered||game.completed||!username)return;const r=game.answer(name);if(!r)return;answered=true;progress=recordAnswer(progress,r.correct);if(r.correct){state.sf++;feedback.textContent=`${t('correct')} +1 SF`;feedback.className='feedback ok';button.classList.add('correct')}else{state.sf--;feedback.textContent=`${t('wrong')} ${r.answer}`;feedback.className='feedback bad';button.classList.add('wrong')}[...answers.children].forEach(b=>{b.disabled=true;if(b.textContent===r.answer)b.classList.add('correct')});saveState(state,username);saveProgress(progress,username);saveIncompleteGame(username,game);stats();showInfo(r.answer);next.hidden=false}
...
function newGame(){if(!username)return;state=refreshDailyEnergy(loadState(username),username);if(state.energy<=0){stats();return}clearIncompleteGame(username);state.energy--;saveState(state,username);game.reset();progress=recordGame(progress);saveProgress(progress,username);showGame();game.next();saveIncompleteGame(username,game);stats()}
function resume(){const s=loadIncompleteGame(username);if(!s||!game.restore(s)){clearIncompleteGame(username);stats();return}showGame();restore();saveIncompleteGame(username,game)}
loginForm.addEventListener('submit',async e=>{e.preventDefault();const u=usernameInput.value.trim().toLowerCase(),k=key.value.trim();if(!/^[a-z0-9.-]{3,32}$/.test(u)||!k){loginFeedback.textContent=t('invalidPostingKey');return}try{await verifyPostingKey(u,k);postingKey=dsteem.PrivateKey.fromString(k);username=setStoredUsername(u);key.value='';state=refreshDailyEnergy(loadState(username),username);progress=loadProgress(username);showHome();logout.hidden=false;refreshSteemBalance(username,steem)}catch{postingKey=null;loginFeedback.textContent=t('invalidPostingKey')}});
logout.addEventListener('click',()=>{clearStoredUsername();username=null;postingKey=null;game.reset();showLoggedOut()});function showLoggedOut(){loginView.hidden=false;home.hidden=true;lb.hidden=true;gameView.hidden=true;logout.hidden=true;hideInfo();stats()}lang.addEventListener('change',()=>{setLanguage(lang.value);applyLanguage(document,getLanguage());if(!gameView.hidden&&game.current)restore()});newBtn.onclick=newGame;resumeBtn.onclick=resume;next.onclick=()=>{if(game.isComplete()){clearIncompleteGame(username);if(game.score>0){const reward={eventId:`${username}-${Date.now()}-${crypto.randomUUID()}`,username,sf:game.score};broadcastSFReward(reward)}else{showHome()}return}game.next();saveIncompleteGame(username,game)};menuBtn.onclick=()=>{menu.hidden=!menu.hidden};document.querySelectorAll('[data-action="home"]').forEach(b=>b.onclick=showHome);document.querySelectorAll('[data-action="reset"]').forEach(b=>b.onclick=()=>{clearIncompleteGame(username);state=refreshDailyEnergy(resetState(username),username);game.reset();showHome()});applyLanguage(document,getLanguage());if(username){state=refreshDailyEnergy(loadState(username),username);showHome();logout.hidden=false}else showLoggedOut();stats()}
bootstrap();
