...
function render(rows, me) {
  ...
  const mine = me ? `<div class="leaderboardMe"><img class="leaderAvatar" src="${escapeAttribute(me.avatar || avatarUrl(me.username))}" alt="@${escapeAttribute(me.username)}"><div><strong>Your Rank: #${Number(me.rank).toLocaleString()}</strong><span>Your SF: ${Number(me.sf ?? 0).toLocaleString()} SF</span></div></div>` : '';
...
async function loadAvatars(rows) {
  ...
  try {
    ...
    return rows.map(row => {
      ...
      return { ...row, avatar: row.avatar || profile.profile_image || avatarUrl(row.username) };
    });
...
async function syncLocalSF() {
  ...
  try {
    ...
    if (state && Number.isSafeInteger(Number(state.sf))) await window.submitGlobalSF(username, Number(state.sf));
  } catch (error) { console.warn('SF leaderboard sync failed:', error); }
}
async function loadLeaderboard() {
  await syncLocalSF();
  const username = currentUser();
  const endpoint = `${API_BASE}/api/leaderboard?limit=100${username ? `&username=${encodeURIComponent(username)}` : ''}`;
  try {
    ...
    render(await loadAvatars(rows), me);
