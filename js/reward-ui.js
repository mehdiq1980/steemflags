...
function check(){const q=new URLSearchParams(location.search);if(q.get('sponsorReward')==='2'){const data=readRewards();if(data){data.adReward=2;data.pendingSave=false;try{localStorage.setItem(REWARD_STATE_KEY,JSON.stringify(data))}catch{}}window.history.replaceState({},document.title,location.pathname+location.hash)}detectFinishedGame();const data=readRewards();const modal=document.getElementById('rewardModal');if(data&&modal)showRewards(data)}
