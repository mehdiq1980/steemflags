...
app.get('/api/leaderboard', async (req, res) => {
  ...
  try {
    ...
    console.error(error); res.status(503).json({ error: 'Leaderboard unavailable' });
  }
});
...
app.post('/api/leaderboard', async (req, res) => {
  ...
  if (!USERNAME_RE.test(username) || !Number.isInteger(sf) || sf < 0 || sf > 1000000000) return res.status(400).json({ error: 'Invalid username or SF' });
  try {
